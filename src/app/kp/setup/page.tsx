...
export default function KpSetupPage() {
  ...
  useEffect(() => {
    // Invite links put tokens in the URL hash (implicit flow).
    // If PKCE is enabled the hash may be absent and a ?code= param is used instead.
      ...
      supabase.auth
        ...
        .then(({ error }) => {
          if (error) {
            ...
            // Clear tokens from the URL bar so they can't be reused via copy-paste.
    ...
    // Fallback: PKCE flow — token arrives as ?code= query param.
  ...
  function submit(e: React.FormEvent) {
    ...
    start(async () => {
      ...
      setPhase("done");
      setTimeout(() => router.push("/kp/dashboard"), 1200);
    });
  }
